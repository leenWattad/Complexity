 ...
 *)
 ...
 module Tag_Parser : TAG_PARSER = struct
   ...
   let rec tag_parse sexpr =
     ...
     (* add support for if *)
     | ScmPair(ScmSymbol "if", ScmPair(tst, ScmPair(dit, ScmNil))) ->
        ScmIf(tag_parse tst, tag_parse dit, ScmConst ScmVoid)
     | ScmPair(ScmSymbol "if", ScmPair(tst, ScmPair(dit, ScmPair(dif, ScmNil)))) ->
        ScmIf(tag_parse tst, tag_parse dit, tag_parse dif)
     ...
     | ScmPair(ScmSymbol "begin", ScmNil) ->
        ScmConst ScmVoid
     | ScmPair(ScmSymbol "begin", ScmPair(e, ScmNil)) ->
        tag_parse e
     | ScmPair(ScmSymbol "begin", exprs) ->
        (match scheme_list_to_ocaml exprs with
         | (lst, ScmNil) ->
            ScmSeq(List.map tag_parse lst)
     ...
     | ScmPair(ScmSymbol "define", ScmPair(ScmPair(var, params), body)) ->
        let lam = ScmPair(ScmSymbol "lambda", ScmPair(params, body)) in
        let expanded = ScmPair(ScmSymbol "define", ScmPair(var, ScmPair(lam, ScmNil))) in
        tag_parse expanded
     | ScmPair(ScmSymbol "define", ScmPair(ScmSymbol v, ScmPair(x, ScmNil))) ->
        if is_reserved_word v
        then raise (X_syntax "cannot define a reserved word")
        else ScmVarDef(Var v, tag_parse x)
     ...
     | ScmPair(ScmSymbol "let", ScmPair(ribs, exprs)) ->
        ...
        if ribs_last <> ScmNil then
          raise (X_syntax "invalid lambda-ribs");
 
        let vars =
          List.map (function
            | ScmPair(v, ScmPair(e, ScmNil)) -> v
            | _ -> raise (X_syntax "invalid lambda-rib structure"))
            ribs_list
        in
        let vals =
          List.map (function
            | ScmPair(_, ScmPair(e, ScmNil)) -> e
            | _ -> raise (X_syntax "invalid lambda-rib structure"))
            ribs_list
        in
        let vars_list =
          List.fold_right (fun v acc -> ScmPair(v, acc)) vars ScmNil
        in
        let vals_list =
          List.fold_right (fun v acc -> ScmPair(v, acc)) vals ScmNil
        in
        let expanded =
          ScmPair(
            ScmPair(ScmSymbol "lambda",
                    ScmPair(vars_list, exprs)),
            vals_list
          )
        in
        tag_parse expanded
     ...
     (* add support for let* *)
     | ScmPair(ScmSymbol "let*", ScmPair(ScmNil, ex)) ->
        tag_parse (ScmPair(ScmSymbol "let", ScmPair(ScmNil, ex)))
     ...
     | ScmPair(ScmSymbol "let*",
               ScmPair(
                 ScmPair(ScmPair(v, ScmPair(a, ScmNil)), ribs),
                 body)) ->
        let expansion =
          ScmPair(
            ScmSymbol "let",
            ScmPair(
              ScmPair(ScmPair(v, ScmPair(a, ScmNil)), ScmNil),
              ScmPair(
     ...
     | ScmPair(ScmSymbol "letrec", ScmPair(ribs, exprs)) ->
        let rec to_scheme_list = function
          | [] -> ScmNil
          | x :: xs -> ScmPair(x, to_scheme_list xs)
        in
        let (ribs_list, ribs_end) = scheme_list_to_ocaml ribs in
        if ribs_end <> ScmNil then
          raise (X_syntax "invalid letrec rib list");
        let bindings =
          List.map (fun rib ->
            match rib with
            | ScmPair(v, ScmPair(val_e, ScmNil)) ->
                ScmPair(
                  v,
                  ScmPair(
                    ScmPair(ScmSymbol "quote", ScmPair(ScmSymbol "whatever", ScmNil)),
                    ScmNil))
            | _ -> raise (X_syntax "Invalid letrec binding1"))
            ribs_list
        in
        let assignments =
          List.map (fun rib ->
            ...
            | ScmPair(v, ScmPair(val_e, ScmNil)) ->
                ScmPair(
                  ScmSymbol "set!",
                  ScmPair(v, ScmPair(val_e, ScmNil)))
            | _ -> raise (X_syntax "Invalid letrec binding2"))
            ribs_list
        in
        let (exprs_list, exprs_end) = scheme_list_to_ocaml exprs in
        let combined =
          List.fold_right (fun se acc -> ScmPair(se, acc)) assignments
            (to_scheme_list exprs_list)
        in
        let expanded_let =
          ScmPair(
            ScmSymbol "let",
            ScmPair(to_scheme_list bindings, combined))
        in
        tag_parse expanded_let
 ...
 module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
   ...
   let annotate_tail_calls = 
     let rec run in_tail = function
       ...
       | ScmLambda'(ps, kind, body) ->
          ScmLambda'(ps, kind, run true body)
       (* add support for applic *)
       | ScmApplic'(p, args, _) ->
          let p1 = run false p in
          let a1 = List.map (fun x -> run false x) args in
          let ak = if in_tail then Tail_Call else Non_Tail_Call in
          ScmApplic'(p1, a1, ak)
   ...
   let rec auto_box expr =
     ...
     (* add support for lambda-opt *)
     | ScmLambda'(ps, Opt opt, bod) ->
        let all = ps @ [opt] in
        let must_box =
          List.filter (fun nm -> should_box_var nm bod all) all
        in
        let new_bod =
          List.fold_left (fun b nm -> box_sets_and_gets nm b)
                         (auto_box bod)
                         must_box
        in
        let sets = make_sets must_box all in
        let final =
          match must_box, new_bod with
          | [], _ -> new_bod
          | _, ScmSeq' seq -> ScmSeq'(sets @ seq)
          | _, any -> ScmSeq'(sets @ [any])
        in
        ScmLambda'(ps, Opt opt, final)
     | ScmApplic' (proc, args, app_kind) ->
	...
	(*"FINAL PROJECT"*)
      ...
      | ScmLambda' (params', Simple, body) ->
         ...
         | ScmLambda' (params', Opt opt, body) ->
          ...
          and label_loop_params_last = make_lambda_opt_loop_params () 
          and label_loop_params_last_end = make_lambda_opt_loop_params_end ()
          ...
          ^ (Printf.sprintf "\tje %s ;  no opt params\n" label_arity_exact) 
          ^ (Printf.sprintf "\tjg %s ;  at least one opt param\n" label_arity_more) 
          ...
          ^ (Printf.sprintf "%s:\n" label_opt_loop_exit)
          ^ (Printf.sprintf "\tmov [rsp + 8 * 2 + 8 * %d], rax\n" ((List.length params') + 1)) 
          ^ (Printf.sprintf "\tmov r15, %d\n" ((List.length params') + 1)) 
          ^ "\tmov [rsp + 8 * 2], r15 ; new params length\n" 
          ^ "\tmov r14, rsp\n"
          ^ (Printf.sprintf "\tadd r14, 8 * 2 + 8 * %d\n" ((List.length params') + 1))
          ^ "\tsub r13, [rsp + 8 * 2] ; \n" 
          ^ "\tmov r15, r13\n"
          ^ "\tshl r15, 3\n" 
          ^ "\tmov rbx, r15\n"
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) ->
          let push_args_code =
            ...
            |> String.concat ""
          in
          let proc_eval_code = run params env proc in
          "\t; prepping a non-tail-call\n"
          ^ push_args_code
          ^ (Printf.sprintf "\tpush %d\t; number of args\n" (List.length args))
          ^ proc_eval_code
